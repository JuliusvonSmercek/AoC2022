...
long long solve(vector<string> &lines) {
  ...
  ll modVal = 1;
  for (Monkey &monkey : monkeys) modVal *= monkey.conditionValue;

  vector<ll> counter(monkeys.size(), 0);
  for (ll i = 0; i < 10000; ++i) {
    for (ll pos = 0; Monkey & monkey : monkeys) {
      while (!monkey.items.empty()) {
        ll item = monkey.items.front();
        monkey.items.pop();
        item = monkey.operation(item) % modVal;
