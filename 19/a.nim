...
for line in load().splitLines:
  ...
  let geodes = blueprint.solve(Material(ore: max robots.mapIt(it.ore), clay: max robots.mapIt(it.clay), obsidian: max robots.mapIt(it.obsidian), geode: high(int)), 24)
  result += ints[0] * geodes
echo result